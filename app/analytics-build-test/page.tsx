...
export default async function AnalyticsBuildTestPage() {
  ...
  const ingestUrl = 'https://lufvkrnwqbqdaqcgljxt.supabase.co/functions/v1/planetx-analytics-ingest'
  ...
  const res = await fetch(ingestUrl, {
    ...
    body: JSON.stringify({
      ...
      platform: 'server',
      properties: { verification: true, ingress: 'supabase' },
    }),
    cache: 'no-store',
  })

  const response = await res.text()
  if (!res.ok) throw new Error(`ANALYTICS_VERIFY: ingest ${res.status}: ${response.slice(0, 300)}`)

  console.log(`ANALYTICS_VERIFY: success ${res.status}: ${response.slice(0, 300)}`)
