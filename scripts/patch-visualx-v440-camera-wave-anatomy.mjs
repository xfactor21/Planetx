...
// ---------------------------------------------------------------------------
// COMPONENT ANATOMY CLEANUP — preserve native per-part choreography while
// reducing whole-world motion that makes otherwise independent parts read as
// one vibrating object. Add another independent lane to Causeway.
// ---------------------------------------------------------------------------
...
world = replaceIn(
  ...
  'this.root.rotation.y+=dt*(.0005+l.mid*.0012);',
  'monolith root motion reduced to near-static stage',
)
...
app = replaceIn(
  ...
  "<aside className='space-y-5'><div className='px-1 text-[9px] font-bold uppercase tracking-[.2em] text-zinc-600'>Visual.X v4.4.0</div>",
  'right drawer build label',
)
