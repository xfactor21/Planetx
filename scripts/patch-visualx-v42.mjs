...
// The media toggle already performs connect() + play()/pause(). Do not hard-disable it
// behind the derived `ready` flag: some mobile/in-app browsers delay onCanPlay even
// though the media element can successfully play when directly invoked by a user gesture.
replace(
  "if (track && ready) void toggle(); }} onKeyDown={e => { if ((e.key === 'Enter' || e.key === ' ') && track && ready) { e.preventDefault(); void toggle(); } }}",
  "if (track) void toggle(); }} onKeyDown={e => { if ((e.key === 'Enter' || e.key === ' ') && track) { e.preventDefault(); void toggle(); } }}",
  'viewer playback readiness gating',
)
replace(
  "<button className={!playing ? 'playActive' : ''} onClick={() => { if (!playing) void toggle(); }} disabled={!ready} aria-label='Play'><Play size={18} /></button><button className={playing ? 'pauseActive' : ''} onClick={() => { if (playing) void toggle(); }} disabled={!ready} aria-label='Pause'><Pause size={18} /></button>",
  "<button className={!playing ? 'playActive' : ''} onClick={() => { if (!playing) void toggle(); }} aria-label='Play'><Play size={18} /></button><button className={playing ? 'pauseActive' : ''} onClick={() => { if (playing) void toggle(); }} aria-label='Pause'><Pause size={18} /></button>",
  'play pause button readiness gating',
)
