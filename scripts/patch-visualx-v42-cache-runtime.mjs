...
replace(
  ...
  "  useEffect(() => { api.get('/api/catalog/status').then(r => setCatalogConfigured(Boolean(r.data?.configured))).catch(() => setCatalogConfigured(false)); }, []);\n  useEffect(()=>{let cancelled=false;const warm=async()=>{for(const item of featuredTracks){if(cancelled)break;try{await analyzeFeatured(item)}catch{}}};const timer=window.setTimeout(()=>void warm(),250);return()=>{cancelled=true;window.clearTimeout(timer)}},[]);",
...
replace(
  "<div className='prototypeTagline'><span>Your Song.</span><small>A World That Only Exists Once.</small></div>",
  "<div className='prototypeTagline'><span>Your Song</span><small>A world that only exists once.</small></div>",
  'exact tagline',
)
...
console.log('Patched Visual.X runtime featured SongDNA cache + persistent xFactor credit')
