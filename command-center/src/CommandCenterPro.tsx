...
export default function CommandCenterPro() {
  ...
  return <div className='cc-shell'>
    ...
    <aside className={`cc-rail ${menu ? 'open' : ''}`}>
      <div className='cc-brand'><div className='cc-x'>X</div><div><b>planet.X</b><span>Command Center / v3.3</span></div><button className='cc-close' onClick={() => setMenu(false)} aria-label='Close navigation'><X /></button></div>
...
function SearchCenter({ google }: { google: GoogleData | null }) {
  ...
  const latestSearchDate = search?.trend?.length ? search.trend[search.trend.length - 1]?.date : undefined
  const snapshotAgeHours = google?.snapshotCapturedAt ? Math.max(0, Math.round((Date.now() - Date.parse(google.snapshotCapturedAt)) / 3600000)) : null
  return <>
    ...
    {tab === 'overview' && <section className='cc-grid'><Panel wide eyebrow='SEARCH TREND' title='Clicks & impressions'><SearchTrend rows={search?.trend || []} /></Panel><Panel eyebrow='ZERO-CLICK OPPORTUNITY' title='Visible pages not earning clicks'><SearchRows rows={zeroClick} /></Panel><Panel eyebrow='GOOGLE SYNC' title='Data freshness'><div className='cc-status-stack'><StatusRow label='Automatic refresh' state={google?.status === 'connected' ? 'connected' : google?.status || 'unknown'} value='Every 6 hours' /><StatusRow label='Snapshot refreshed' value={google?.snapshotCapturedAt ? new Date(google.snapshotCapturedAt).toLocaleString() : '—'} /><StatusRow label='Snapshot age' value={snapshotAgeHours === null ? '—' : snapshotAgeHours < 1 ? '< 1 hour' : `${snapshotAgeHours} hours`} /><StatusRow label='Latest settled GSC day' value={latestSearchDate || '—'} /></div></Panel><Panel eyebrow='GA4' title='Audience status'>{google?.analytics?.status === 'connected' ? <div className='cc-status-stack'><StatusRow label='GA4 connection' state='connected' /><StatusRow label='Sessions' value={fmt(google.analytics.totals?.sessions)} /><StatusRow label='Events' value={fmt(google.analytics.totals?.events)} /><StatusRow label='Page views' value={fmt(google.analytics.totals?.pageViews)} /></div> : <Empty text='GA4 is not currently returning data.' />}</Panel></section>}
