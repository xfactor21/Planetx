...
function rangeEnding(days:number,endDate:string){const end=day(endDate);const start=new Date(end);start.setUTCDate(start.getUTCDate()-days+1);const prevEnd=new Date(start);prevEnd.setUTCDate(prevEnd.getUTCDate()-1);const prevStart=new Date(prevEnd);prevStart.setUTCDate(prevStart.getUTCDate()-days+1);return{current:{startDate:iso(start),endDate:iso(end)},previous:{startDate:iso(prevStart),endDate:iso(prevEnd)}}}
function lagDays(finalDate:string){const today=day(iso(new Date()));return Math.max(0,Math.round((today.getTime()-day(finalDate).getTime())/DAY_MS))}
...
async function latestFinalDay(endpoint:string,t:string){const end=new Date(),start=new Date(end);start.setUTCDate(start.getUTCDate()-14);try{const data=await query(endpoint,t,{startDate:iso(start),endDate:iso(end)},['date'],30);const dates=(data.rows||[]).map((r:MetricRow)=>r.keys?.[0]).filter((value:unknown):value is string=>typeof value==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(value)).sort();if(dates.length)return{date:dates[dates.length-1],detected:true}}catch{}const fallback=new Date();fallback.setUTCDate(fallback.getUTCDate()-3);return{date:iso(fallback),detected:false}}
...
function aggregatePageGroups(rows:MetricRow[]=[]){const groups=PAGE_GROUPS.map(group=>{const picked=rows.filter(r=>group.match(pathOf(r.keys?.[0]||'')));return{name:group.name,...summarize(picked)}});const core=rows.filter(r=>!PAGE_GROUPS.some(group=>group.match(pathOf(r.keys?.[0]||''))));return[...groups,{name:'Core Site',...summarize(core)}]}
...
export default async function handler(req:any,res:any){
  ...
  try{
    const token=await tokenFor(c),site=c.search_console_site_url||DEFAULT_SITE,days=Math.max(7,Math.min(90,Number(req.query?.days)||28)),endpoint=`https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(site)}/searchAnalytics/query`;
    const settled=await latestFinalDay(endpoint,token),dates=rangeEnding(days,settled.date);
    ...
    return json(res,200,{
      ...
      freshness:{finalizedThrough:settled.date,lagDays:lagDays(settled.date),detectedFromFinalRows:settled.detected,note:settled.detected?'Main comparisons end on the latest date Google returned as finalized. The 24-hour pulse is explicitly partial.':'No recent finalized date row was returned, so the comparison window uses a conservative three-day fallback. The 24-hour pulse remains partial.'},
